...
class model(object):
    ...
    def __init__(self, nh, nc, ne, de, cs):
        ...
        self.V_i = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0, (nh, nh)).astype(theano.config.floatX)) #cell state vector
        ...
        self.V_f = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0, (nh, nh)).astype(theano.config.floatX))
        ...
        self.V_o = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0, (nh, nh)).astype(theano.config.floatX))
        ...
        # bundle weights
        self.params = [self.emb, self.W_i, self.U_i, self.V_i, self.b_i, self.W_f, self.U_f, \
                       self.V_f, self.b_f, self.W_c, self.U_c, self.b_c, self.W_o, self.U_o, \
                       self.V_o, self.b_o, self.V, self.b, self.c0]
        self.names  = ['embeddings', 'W_i', 'U_i', 'V_i', 'b_i', 'W_f', 'U_f', 'V_f', 'b_f', \
                       'W_c', 'U_c', 'b_c', 'W_o', 'U_o', 'V_o', 'b_o', 'V', 'b', 'c0']
        ...
        def recurrence(x_t, h_tm1, c_tm1):
            ...
            i_t = T.nnet.sigmoid(T.dot(x_t, self.W_i) + T.dot(h_tm1, self.U_i) + T.dot(c_tm1, self.V_i) + self.b_i)
            f_t = T.nnet.sigmoid(T.dot(x_t, self.W_f) + T.dot(h_tm1, self.U_f) + T.dot(c_tm1, self.V_f) + self.b_f)
            c_t = f_t * c_tm1 + i_t * T.tanh(T.dot(x_t, self.W_c) + T.dot(h_tm1, self.U_c) + self.b_c)
            o_t = T.nnet.sigmoid(T.dot(x_t, self.W_o)+ T.dot(h_tm1, self.U_o) + T.dot(c_tm1, self.V_o)  + self.b_o)
            h_t = o_t * T.tanh(c_t)
